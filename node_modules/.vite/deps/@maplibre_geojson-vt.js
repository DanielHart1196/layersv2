...
function convertToInternal(data, options) {
  ...
  switch (data.type) {
    ...
    case "Feature":
      ...
      break;
    default:
      featureToInternal(features, { geometry: data, properties: void 0 }, options);
  }
  return features;
}
...
var AxisType;
(function(AxisType2) {
  AxisType2[AxisType2["X"] = 0] = "X";
  AxisType2[AxisType2["Y"] = 1] = "Y";
})(AxisType || (AxisType = {}));
function clip(features, scale, start, end, axis, minAll, maxAll, options) {
  start /= scale;
  end /= scale;
  if (minAll >= start && maxAll < end) {
    return features;
  }
  if (maxAll < start || minAll >= end) {
    return null;
  }
  const clipped = [];
...
function clipLine(geom, newGeom, start, end, axis, isPolygon, trackMetrics) {
  ...
  for (let i = 0; i < geom.length - 3; i += 3) {
    ...
    const bx = geom[i + 3];
    const by = geom[i + 4];
    const a2 = axis === AxisType.X ? ax2 : ay2;
    ...
    if (a2 < start) {
      if (b > start) {
        t = intersect(slice, ax2, ay2, bx, by, start);
        if (trackMetrics)
          slice.start = len + segLen * t;
      }
    } else if (a2 > end) {
      if (b < end) {
        t = intersect(slice, ax2, ay2, bx, by, end);
        if (trackMetrics)
          slice.start = len + segLen * t;
      }
    } else {
...
var GEOJSONVT_CLIP_START = "geojsonvt_clip_start";
var GEOJSONVT_CLIP_END = "geojsonvt_clip_end";
...
var TileIndex = class {
  ...
  invalidateTiles(features) {
    ...
    for (const id in this.tiles) {
      ...
      for (const feature of features) {
        if (feature.maxX >= tileMinX && feature.minX < tileMaxX && feature.maxY >= tileMinY && feature.minY < tileMaxY) {
          intersects = true;
          break;
        }
      }
...
//# sourceMappingURL=@maplibre_geojson-vt.js.map
