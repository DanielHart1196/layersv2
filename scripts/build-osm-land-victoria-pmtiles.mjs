...
function buildTargetPath(tileId) {
  return path.join(PUBLIC_DIR, `osm-land-victoria-${tileId}.pmtiles`);
}
...
for (const tile of VICTORIA_TILES) {
  ...
  runOgr2ogr([
    ...
    "land-fill",
    "-dsco",
    `NAME=OSM Land Victoria ${tile.id.toUpperCase()}`,
    "-dsco",
    `DESCRIPTION=Victoria-only OSM land tile ${tile.id.toUpperCase()} for atlas-product high-detail fill testing`,
    "-dsco",
    "MINZOOM=0",
    "-dsco",
    "MAXZOOM=14",
...
console.log(`Built ${VICTORIA_TILES.length} Victoria land PMTiles slices`);
