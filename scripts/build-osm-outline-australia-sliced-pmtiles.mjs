...
function buildTargetPath(tileId) {
  return path.join(PUBLIC_DIR, `osm-outline-australia-${tileId}.pmtiles`);
}
...
for (const tile of AUSTRALIA_TILES) {
  ...
  runOgr2ogr([
    ...
    "coastlines",
    "-dsco",
    `NAME=OSM Coastlines Australia ${tile.id.toUpperCase()}`,
    "-dsco",
    `DESCRIPTION=Australia OSM coastlines slice ${tile.id.toUpperCase()} for atlas-product high-detail outline testing`,
    "-dsco",
    "MINZOOM=0",
    "-dsco",
    "MAXZOOM=14",
...
console.log(`Built ${AUSTRALIA_TILES.length} sliced Australia outline PMTiles archives`);
