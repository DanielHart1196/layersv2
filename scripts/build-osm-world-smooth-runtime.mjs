...
const LAND_OUTPUT_PATH = path.join(WORLD_ATLAS_DIR, "osm-land-fill.smooth.geojson");
const COASTLINES_OUTPUT_PATH = path.join(WORLD_ATLAS_DIR, "osm-coastlines.smooth.geojson");

const LAND_TOLERANCE_DEGREES = 0.008;
const COASTLINE_TOLERANCE_DEGREES = 0.004;
const LAND_BATCH_SIZE = 1024;
const COASTLINE_BATCH_SIZE = 4096;
...
async function buildLandRuntime() {
  ...
  const writer = await createFeatureCollectionWriter(LAND_OUTPUT_PATH, {
    ...
    generatedBy: "scripts/build-osm-world-smooth-runtime.mjs",
  ...
  while (true) {
    ...
    for (const polygon of polygons) {
      ...
      if (batch.length >= LAND_BATCH_SIZE) {
        ...
        writer.writeFeature({
          type: "Feature",
          id: `osm-land-smooth-${featureCount}`,
    ...
    if (recordCount % 50000 === 0) {
      console.log(`Smooth land runtime: processed ${recordCount} records`);
    }
  }
  ...
  while (true) {
    ...
    for (const line of lines) {
      ...
      if (batch.length >= COASTLINE_BATCH_SIZE) {
        ...
        writer.writeFeature({
          type: "Feature",
          id: `osm-coastline-smooth-${featureCount}`,
    ...
    if (recordCount % 50000 === 0) {
      console.log(`Smooth coastline runtime: processed ${recordCount} records`);
    }
  }
