...
create table layers (
  ...
  -- Original uploaded file and generated tiles, stored in Supabase Storage.
  file_url          text,
  tiles_url         text,
...
create policy "anon can insert features for anon layers"
  ...
  with check (
    exists (
      select 1 from layers
      where id = features.layer_id and owner_id is null
    )
  );
...
create policy "owners can delete features"
  ...
  using (
    exists (
      select 1 from layers
      where id = features.layer_id and owner_id = auth.uid()
    )
  );

-- Keep feature_count in sync
create or replace function sync_feature_count()
...
create trigger on_feature_change
  ...
  for each row execute function sync_feature_count();
...
begin
  ...
  if v_access = 'open' then
    ...
    insert into features (layer_id, geometry, properties, valid_from, valid_to)
    values (
      NEW.layer_id,
      v_geom,
...
-- ─── Storage ──────────────────────────────────────────────────────────────────
-- Public bucket for raw uploaded files and generated PMTiles.
